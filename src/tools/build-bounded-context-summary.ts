...
const CHEMIN_VERS_LE_TEMPLATE = resolve(
...
async function genererLeBoundedContextSummary(
  nomDuBoundedContext: string
): Promise<void> {
...
function creerLeBoundedContextSummary(boundedContext: IBoundedContext): string {
  const templateDeBoundedContextSummary = readFileSync(
    CHEMIN_VERS_LE_TEMPLATE,
    { encoding: "utf8" }
  );
  ...
  return templateDeBoundedContextSummary
    ...
    .replace(
      "{{ businessModel }}",
      boundedContext.classificationStrategique.businessModel
    )
    .replace(
      "{{ evolution }}",
      boundedContext.classificationStrategique.evolution
    )
    .replace("{{ rolesDuDomaine }}", boundedContext.rolesDuDomaine.join(", "))
    .replace(
      "{{ questions }}",
      boundedContext.questions.map(versDetails).join(EOL)
    )
    .replace(
      "{{ modelesDeLecture }}",
      boundedContext.modelesDeLecture.map(versDetails).join(EOL)
    )
    .replace(
      "{{ commandes }}",
      boundedContext.commandes.map(versDetails).join(EOL)
    )
    .replace(
      "{{ evenementsDuDomaine }}",
      boundedContext.evenementsDuDomaine.map(versDetails).join(EOL)
    )
    .replace(
      "{{ ubiquitousLanguage }}",
...
function ecrireBoundedContextSummarySurLeSystemeDeFichier(
  ...
  writeFileSync(cheminVersLeBoundedContextSummary, boundedContextSummary, {
    encoding: "utf8",
  });
}
