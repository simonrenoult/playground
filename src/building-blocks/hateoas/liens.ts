...
export default class Liens {
  ...
  public creer(messageInitial: Message): Lien[] {
    const mappingHttpDuMessageInitial = this.mappingHttpDesMessages.find(
      (mappingHttpDUnMessage) =>
        mappingHttpDUnMessage.message.name === messageInitial.constructor.name
    );
    if (!mappingHttpDuMessageInitial)
      throw new Error(
        "Pas de mapping http trouvé pour le message " + messageInitial.nom
      );
    ...
    const liensSuivants = this.arborescencesDeMessages
      .find(
        (arborescenceDeMessages: ArborescenceDeMessages) =>
          arborescenceDeMessages.messageInitial.name ===
          messageInitial.constructor.name
      )
      .messagesSuivants.map((message: Constructor<Message>) => {
        const mappingHttpDuMessage = this.mappingHttpDesMessages.find(
          (mappingHttpDUnMessage) =>
