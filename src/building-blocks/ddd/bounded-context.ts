...
export type ArborescenceDeMessages = {
  messageInitial: Constructor<Message>;
  messagesSuivants: Constructor<Message>[];
};
...
export default class BoundedContext implements IBoundedContext {
  ...
  public constructor(
    ...
    const elements = BoundedContext.listerLesElementsDuBoundedContext(
      cheminVersLaRacineDuBoundedContext
    );
    this.ubiquitousLanguage =
      BoundedContext.recupererUbiquitousLanguage(elements);
    this.questions = BoundedContext.recupererLesQuestions(elements);
    this.modelesDeLecture =
      BoundedContext.recupererLesModelesDeLecture(elements);
    this.commandes = BoundedContext.recupererLesCommandes(elements);
    this.evenementsDuDomaine =
      BoundedContext.recupererLesEvenementsDuDomaine(elements);
  }

  private static recupererUbiquitousLanguage(
    classes: Array<ClassDeclaration | InterfaceDeclaration>
  ): string[] {
    ...
    const valueObjects = recupererLeDetailDeLElementPourLeType(
      classes,
      "ValueObject"
    );
    const repositories = recupererLeDetailDeLElementPourLeType(
      classes,
      "Repository"
    );
  ...
  private static listerLesElementsDuBoundedContext(
    cheminVersLeBoundedContext: string
  ): Array<ClassDeclaration | InterfaceDeclaration> {
    const projet = new Project({
      tsConfigFilePath: resolve(__dirname, "../../..", "tsconfig.json"),
    });
    const fichiers = projet
      .getSourceFiles()
      .filter((sourceFile) =>
        sourceFile.getFilePath().includes(cheminVersLeBoundedContext)
      );
    return fichiers.flatMap((sf) => [
      ...sf.getClasses(),
      ...sf.getInterfaces(),
    ]);
  }
...
function recupererLeDetailDeLElementPourLeType(
  elements: Array<ClassDeclaration | InterfaceDeclaration>,
  interfaceImplementee: Interfaces
): Array<DetailsDeLaClasse> {
  return elements
    .filter((c) => {
      if (c instanceof ClassDeclaration) {
        return c
          .getImplements()
          .some((i) => i.getText().startsWith(interfaceImplementee));
      }

      if (c instanceof InterfaceDeclaration) {
        return c
          .getExtends()
          .some((e) => e.getText().startsWith(interfaceImplementee));
      }
