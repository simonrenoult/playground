...
export default class SessionsDeFormationEndpoints implements ListeDeEndpoints {
  ...
  enregistrerEndpoints(fastify: FastifyInstance): void {
    ...
    fastify.route<{ Params: { idSession: string }, Body: { emailFormateur: string } }>({
      method: 'POST',
      ...
      handler: async (req) => {
        const message = new AjouterUnFormateurAUneSessionDeFormation(req.body.emailFormateur, req.params.idSession)
        const data = this.busDeCommandes.publier(message)
        return { data, liens: this.constructeurDeLiens.creer(message) }
      }
    })
    ...
    fastify.route<{ Params: Static<typeof IdSessionWrapper>, Body: Static<typeof EmailParticipantWrapper> }>({
      method: 'POST',
      ...
      handler: async (req) => {
        const message = new InscrireUnParticipantAUneSessionDeFormation(req.body.emailParticipant, req.params.idSession)
        const data = this.busDeCommandes.publier(message)
        return { data, liens: this.constructeurDeLiens.creer(message) }
      }
    })
